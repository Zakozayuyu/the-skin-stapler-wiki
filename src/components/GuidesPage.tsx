...
import { getLocalizedArticle, getLocalizedArticlePath, localizedGuideSlugs } from '@/lib/localized-articles';
import { siteConfig } from '@/lib/seo';
import SiteShell from './SiteShell';

const copy = {
  en: { title: 'Investigation Guides', description: 'Practical routes and reference pages for The Skin Stapler, with spoilers and still-unknown details clearly labeled.', open: 'Open guide →', verified: 'Verified', pending: 'Needs verified detail', demoTitle: 'Free Demo', demoText: 'The Skin Stapler has a separate free demo on Steam.', demoButton: 'Steam Demo' },
...
export default function GuidesPage({ locale }: { locale: Locale }) {
  ...
  const primaryRoutes: Record<string, string> = { beginner: localizePath(locale, '/guides/beginner'), walkthrough: localizePath(locale, '/walkthrough'), achievements: localizePath(locale, '/guides/the-skin-stapler-achievements') };
  const localizedGuides = locale === 'en' ? [] : Object.values(localizedGuideSlugs).map((id) => getLocalizedArticle(locale, id)).filter(Boolean);
  return <SiteShell locale={locale}><div className="container games-page"><div className="page-heading"><span className="badge blue-badge">The Skin Stapler Wiki</span><h1>{labels.title}</h1><p>{labels.description}</p></div><div className="games-grid guide-shell-grid">{startCards[locale].map((guide) => primaryRoutes[guide.id] ? <Link href={primaryRoutes[guide.id]} key={guide.id} id={guide.id} className="card game-card"><div><h2>{guide.title}</h2><span className="badge tier-badge">{labels.verified}</span></div><p>{guide.description}</p><b>{labels.open}</b></Link> : <article key={guide.id} id={guide.id} className="card game-card template-card"><div><h2>{guide.title}</h2><span className="badge floor-badge">{labels.pending}</span></div><p>{guide.description}</p></article>)}{locale === 'en' && keywordArticleSlugs.map((slug) => { const article = keywordArticles[slug]; return <Link href={`/guides/${slug}`} key={slug} className="card game-card"><div><h2>{article.title}</h2><span className="badge blue-badge">{article.category}</span></div><p>{article.summary}</p><b>{labels.open}</b></Link>; })}{locale !== 'en' && localizedGuides.map((article) => article && <Link href={getLocalizedArticlePath(locale, article.id)} key={article.id} className="card game-card"><div><h2>{article.title}</h2><span className="badge blue-badge">{article.eyebrow}</span></div><p>{article.description}</p><b>{labels.open}</b></Link>)}</div><div className="guide-demo-cta card"><h2>{labels.demoTitle}</h2><p>{labels.demoText}</p><a href={siteConfig.demo} target="_blank" rel="noreferrer" className="btn-primary">{labels.demoButton}</a></div></div></SiteShell>;
}
